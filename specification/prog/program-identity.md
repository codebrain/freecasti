...
_Generated 2026-07-18. Source folder: `sysex/prog/presets/` (222 dumps named `<bank name>.<preset name>.syx`)._

**Confidence:** high

Program name bytes at offsets 8-87 match filename preset (ASCII space-padded) in 222/222 dumps. Bank index at 88-89 (nibble_hilo); mirrored at 137. Program slot within bank at 90-91 (nibble_hilo).
...
- **Program name** offsets **8-87** (`ascii_space_padded`). Filename check: 222/222 dumps match (mismatch count 0). ASCII program name only - bank name is not stored in this field. Each dump's bytes[8:88] are checked against the filename preset half, encoded as ASCII and space-padded to 80 bytes.
