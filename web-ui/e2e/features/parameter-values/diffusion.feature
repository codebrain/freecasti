...
Feature: Allowable values for "diffusion"
  The device supports a fixed table of 11 steps for
  "diffusion". The editor must offer exactly these values, in this
  order, and nothing else.

  Scenario: Every value the editor may offer for "diffusion"
    Given the editor is started
    Then the only values available for "diffusion" are:
      | Low |
      | 1 |
      | 2 |
      | 3 |
