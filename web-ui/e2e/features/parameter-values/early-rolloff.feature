...
Feature: Allowable values for "early rolloff"
  The device supports a fixed table of 71 steps for
  "early rolloff". The editor must offer exactly these values, in this
  order, and nothing else.

  Scenario: Every value the editor may offer for "early rolloff"
    Given the editor is started
    Then the only values available for "early rolloff" are:
      | 80 Hz |
      | 120 Hz |
      | 160 Hz |
      | 200 Hz |
