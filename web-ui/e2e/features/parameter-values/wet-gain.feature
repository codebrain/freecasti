...
Feature: Allowable values for "wet gain"
  The device supports a fixed table of 122 steps for
  "wet gain". The editor must offer exactly these values, in this
  order, and nothing else.

  Scenario: Every value the editor may offer for "wet gain"
    Given the editor is started
    And the user opens the "system" view
    Then the only values available for "wet gain" are:
      | Off |
      | -60 dB |
      | -59.5 dB |
      | -59 dB |
